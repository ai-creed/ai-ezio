...
`submit` → `false`; `exit` is unreachable (no `/quit` registered in mounted mode).
...
- **`/quit`, `/exit`** — not registered in mounted mode. Typing `/quit` falls to
  the standard local "unknown command: /quit. type /help for the list." message.
  (Decision: plain unknown-command message rather than a tailored "not available
  in mounted mode" line — simpler, and re-evaluated when lifecycle commands are
  designed.)
...
- **Relocation (ai-ezio):** the existing `slash.test.ts`, skills, clipboard, and
  ...
  there. Standalone REPL tests are unchanged except for repointed imports.
- **Adapter (ai-whisper):** unit-test `tryConsumeLocalCommand` with a fake session
  ...
  unknown-command message. Assert `lastContent`/`lastUsage` track
  `assistant_turn_finished`.
- **Host (ai-whisper):** `live-session.test.ts` — a slash operator line is
  consumed (no `writeUserInput`, no echo-stripe); an ordinary operator line still
  submits; **a relayed/injected line never calls `tryConsumeLocalCommand`**
  (regression guard for operator-only scoping).
...
- **Stage 2 — `ai-whisper`:** widen `AiEzioEngineSession`; add the optional
  ...
  build the `SlashController`, add `lastContent`/`lastUsage` tracking, implement
  the seam, and add the mounted-transcript wiring (mint path at start + dump
  mode).
...
**ai-ezio**
- `packages/surface/src/slash.ts` (moved; structural interfaces) + `slash.test.ts`
