...
Verify: `pnpm -r build && pnpm -r test` → protocol 21, harness 87, surface 127,
mcp-host 30, session-recorder 25 (+1 skip), cli 166 = **456 passed / 1 skip**. The hax
...
pnpm exec vitest run test/adapter-ai-ezio-live-session.test.ts test/live-session-runtime.test.ts
                                          # → 48 passed / 0 skipped (incl. the mounted /rename→/resume e2e)
pnpm test                                 # full suite → 257 files, 1583 passed / 3 skip
```
